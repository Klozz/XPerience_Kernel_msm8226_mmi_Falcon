...
#ifndef CONFIG_ALESSA_HOTPLUG_USE_CPU_UTIL
	u64 prev_cpu_wall;
	u64 prev_cpu_idle;
#endif
...
static struct workqueue_struct *alessahp_wq;

static struct delayed_work alessa_hotplug_work;
...
#ifdef CONFIG_ALESSA_HOTPLUG_USE_CPU_UTIL
...
*/
	queue_delayed_work_on(0, alessahp_wq, &alessa_hotplug_work,
							  delay);
}

#if defined(CONFIG_POWERSUSPEND) || defined(CONFIG_HAS_EARLYSUSPEND)
#ifdef CONFIG_POWERSUSPEND
static void __ref alessa_hotplug_suspend(struct power_suspend *handler)
#else
static void __ref alessa_hotplug_early_suspend(struct early_suspend *handler)
#endif
{
...
#ifdef CONFIG_POWERSUSPEND
static void __ref alessa_hotplug_resume(struct power_suspend *handler)
#else
static void __ref alessa_hotplug_late_resume(
...
#ifdef CONFIG_POWERSUSPEND
static struct power_suspend alessa_hotplug_power_suspend_driver = {
	.suspend = alessa_hotplug_suspend,
	.resume = alessa_hotplug_resume,
};
#else
static struct early_suspend alessa_hotplug_early_suspend_driver = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 10,
	.suspend = alessa_hotplug_early_suspend,
	.resume = alessa_hotplug_late_resume,
};
#endif
#endif  /* CONFIG_POWERSUSPEND || CONFIG_HAS_EARLYSUSPEND */
...
static int hotplug_start(void)
{
	...
	alessahp_wq = alloc_workqueue("alessahp_wq", WQ_HIGHPRI, 0);

	if (!alessahp_wq) {
		printk(KERN_ERR "Failed to create alessa hotplug workqueue\n");
		return -EFAULT;
	}

	ret = init_rq_avg();
	if (ret) {
		destroy_workqueue(alessahp_wq);
		return ret;
	}
	...
	init_rq_avg_stats();
	INIT_DELAYED_WORK(&alessa_hotplug_work, hotplug_work_fn);
	queue_delayed_work_on(0, alessahp_wq, &alessa_hotplug_work,
...
#if defined(CONFIG_POWERSUSPEND)
	register_power_suspend(&alessa_hotplug_power_suspend_driver);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	register_early_suspend(&alessa_hotplug_early_suspend_driver);
#endif  /* CONFIG_POWERSUSPEND || CONFIG_HAS_EARLYSUSPEND */
...
#if defined(CONFIG_POWERSUSPEND)
	unregister_power_suspend(&alessa_hotplug_power_suspend_driver);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	unregister_early_suspend(&alessa_hotplug_early_suspend_driver);
#endif  /* CONFIG_POWERSUSPEND || CONFIG_HAS_EARLYSUSPEND */

	cancel_delayed_work_sync(&alessa_hotplug_work);
...
static struct attribute *alessa_hotplug_attributes[] = {
...
static struct attribute_group alessa_hotplug_attr_group = {
	.attrs = alessa_hotplug_attributes,
	.name = "alessa_hotplug",
};

static int __init alessa_hotplug_init(void)
{
	int ret;
	unsigned int cpu;
...
/* here if you port to other device you need set the CPU freq to our device*/
	...
	ret = sysfs_create_group(kernel_kobj, &alessa_hotplug_attr_group);
	if (ret) {
...
static void __exit alessa_hotplug_exit(void)
{
	if (hotplug_tuners_ins.hotplug_enable > 0) {
		hotplug_stop();
	}

	sysfs_remove_group(kernel_kobj, &alessa_hotplug_attr_group);
}
MODULE_AUTHOR("Carlos "Klozz" Jesus TeamMEX@XDA");
MODULE_DESCRIPTION("'Alessa_plug' - A cpu hotplug driver for "
	"capable processors");
MODULE_LICENSE("GPL");
MODULE_VERSION(KH_STP);

late_initcall(alessa_hotplug_init);
/*
module_init(alessa_hotplug_init);
module_exit(alessa_hotplug_exit); 
*/ 